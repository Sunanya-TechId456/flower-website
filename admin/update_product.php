...
if (isset($_POST['update'])) {
   ...
   $update_product = mysqli_prepare($conn, "UPDATE `products` SET name = ?, category = ?, price = ? WHERE id = ?");
   ...
   if (!empty($image)) {
      if ($image_size > 2000000) {
         ...
         $update_image = mysqli_prepare($conn, "UPDATE `products` SET image = ? WHERE id = ?");
...
<section class="update-product">
   ...
   <?php
      ...
      $show_products = mysqli_prepare($conn, "SELECT * FROM `products` WHERE id = ?");
