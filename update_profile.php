...
if (isset($_POST['submit'])) {
   ...
   if (!empty($name)) {
      $select_name = mysqli_prepare($conn, "SELECT * FROM `admin` WHERE name = ?");
      mysqli_stmt_bind_param($select_name, 's', $name);
      mysqli_stmt_execute($select_name);
      $result_name = mysqli_stmt_get_result($select_name);
      
      if (mysqli_num_rows($result_name) > 0) {
         $message[] = 'username already taken!';
      } else {
         $update_name = mysqli_prepare($conn, "UPDATE `admin` SET name = ? WHERE id = ?");
   ...
   $select_old_pass = mysqli_prepare($conn, "SELECT password FROM `admin` WHERE id = ?");
   mysqli_stmt_bind_param($select_old_pass, 'i', $admin_id);
   mysqli_stmt_execute($select_old_pass);
   $result_pass = mysqli_stmt_get_result($select_old_pass);
   $fetch_prev_pass = mysqli_fetch_assoc($result_pass);
   ...
   if ($old_pass != $empty_pass) {
      if ($old_pass != $prev_pass) {
         ...
         if ($new_pass != $empty_pass) {
            $update_pass = mysqli_prepare($conn, "UPDATE `admin` SET password = ? WHERE id = ?");
